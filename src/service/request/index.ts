...
class LycRequest {
  ...
  request<T>(config: LycRequestConfig<T>): Promise<T> {
  ...
  get<T = any>(config: LycRequestConfig<T>): Promise<T> {
    return this.request<T>({ ...config, method: "GET" })
  }

  post<T = any>(config: LycRequestConfig<T>): Promise<T> {
    return this.request<T>({ ...config, method: "POST" })
  }

  delete<T = any>(config: LycRequestConfig<T>): Promise<T> {
    return this.request<T>({ ...config, method: "DELETE" })
  }

  patch<T = any>(config: LycRequestConfig<T>): Promise<T> {
    return this.request<T>({ ...config, method: "PATCH" })
  }

  put<T = any>(config: LycRequestConfig<T>): Promise<T> {
